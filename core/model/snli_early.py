...
class SNLI_Early(nn.Module):
    def __init__(self, args, vocab_size, pretrained_emb):
        ...
        self.pos_embedding_text_hyp = nn.Parameter(torch.randn(1, args.text_len-1, args.dim))
    ...
    def forward(self,pre,hyp,xy_mask):
        ...
        xy = xy[:, 0]
        xy= self.to_latent(xy) 

        return self.mlp_x(xy)

    def feedforward(self,device,feat,net):
        x,y,xy_mask,ans = feat
        x = x.to(device)
        y = y.to(device)
