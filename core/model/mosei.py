...
class SoftGate(nn.Module):
    def __init__(self,args):
        ...
        self.layer = nn.Sequential(
            nn.Linear(2*(self.args.audio_len+self.args.text_len),64),
            nn.ReLU(),
            nn.Dropout(0.1),
            nn.Linear(64,2)
        )
    ...
    def forward(self,x,y,xy_mask):
        ...
        x_representation = co_representation[:,0:self.args.audio_len,:]
        y_representation = co_representation[:,self.args.audio_len:,:]
...
class Mosei(nn.Module):
    def __init__(self, args, vocab_size, pretrained_emb):
        ...
        self.adapter_audio = nn.Linear(80,args.dim)

        self.pos_embedding_audio = nn.Parameter(torch.randn(1, args.audio_len, args.dim))
        self.cls_token_audio = nn.Parameter(torch.randn(1, 1, args.dim))
    ...
    def forward(self,audio,text,audio_mask,text_mask,xy_mask):
       
        """
        shape torch.Size([1, 59, 80]) torch.Size([1, 60]) torch.Size([1]) torch.Size([1, 1, 60]) torch.Size([1, 1, 60])
        
        
        """
        
        x_mask = audio_mask.unsqueeze(1) #b,1,1,len : 
        y_mask = text_mask.unsqueeze(1)
        xy_mask = xy_mask.unsqueeze(1)

      
        # patch embedding 
        x = self.adapter_audio(audio) # b,len,dim 
        b, n, _ = x.shape

        # 1,1,d --> b,1,d 
        cls_tokens_audio = repeat(self.cls_token_audio, '() n d -> b n d', b = b)
        
        x = torch.cat((cls_tokens_audio, x), dim=1) #batch len+1, dim 

        # 1, len+1,dim  # broadcast for each batch 
        x += self.pos_embedding_audio[:, :(n + 1)]
        ...
        y = y_embedding+self.pos_embedding_text[:,:]
    ...
    def feedforward(self,device,feat,net):
        x,y,ans,x_mask,y_mask,xy_mask = feat
        x = x.to(device)
        y = y.to(device)
        ans = ans.to(device)
