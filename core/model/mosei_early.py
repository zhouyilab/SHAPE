...
class Mosei_Early(nn.Module):
    ...
    def forward(self,audio,text,xy_mask):
        ...
        x_embedding = self.adapter_audio(audio)
        
      
        # 1,1,d --> b,1,d 
        b, n, _ = x_embedding.shape
        # 1, num_of_patch+1,dim  # broadcast for each batch 
        x = x_embedding+self.pos_embedding_audio[:, :(n)]
        ...
        input_concat = torch.cat((y,x),dim=1)
        ...
        return self.mlp_x(out)
    def feedforward(self,device,feat,net):
        x,y,ans,xy_mask = feat
        x = x.to(device)
        y = y.to(device)
        ans = ans.to(device)
